  ...
  <body>
    ...
    <div class="navbar">
      <ul class="nav nav-tabs" role="tablist">
        <li class="active nav-tab"><a href="home.php">Buy a Kindle</a></li>
    ...
    <div class="container" id="product-container">
      ...
      if(isset($_POST["title"]))
          {
            ...
            $id=0;
              try
                ...
                try    
                  {
                  $sel1 = $db->prepare("select * from Kindle,Product where Product.Asin=Kindle.Asin");
                  $sel1->execute();
                  $i=1;
      ...
      <div class="product-display">';
          ...
          <div class="page-piece">
            ...
            '.$row['name'].'<br><small>'.$row['Asin'].'</small><br>
            <div class="inline-piece" >
              <div class="orange-heading"> Price<span class="piece-heading"> $'.$row['Price'].'</span> </div>';
                if($row['stock']!=0)
                echo'<div class="piece-heading" style="color:green">STOCK </div>';
              else
                echo'<div class="piece-heading" style="color:red">NOT IN STOCK </div>';
              if(is_null(($row["Rating"])))
              echo'<small>NOT RATED</small>';
            else
              echo'<input id="notchange" class="rating"  data-min="0" data-max="5" data-step="1" data-size="xs" name="rating" value="'.$row["Rating"].'">';    
          ...
          echo'</div>';                          
           echo' </h2>
            ...
            <div class="inline-piece" >';
              try    
                  {
                  $sel2 = $db->prepare("show columns from Kindle");
                  $sel2->execute();
                  $ro = $sel2->fetchAll();
                  foreach ($ro as $ro1)
                  {
              echo'<div class="page-piece" style="border:none;">';
                  if(!is_null($row[$ro1["Field"]]))
